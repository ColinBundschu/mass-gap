...
/-!
The drift tier prices the ground's electric weight at the drift,
the join of the ground level with the top root (`drift_cap`).
...
`A` and `p` (the tex's `℘`; `capA` the halved summands' scaled residue product,
...
whole power (the `gradSum` reads, `lem:contactdrift`'s two displayed
graded-sum reads).
...
/-! `lem:contactdrift`'s contact drift: the ground's electric weight rides
...
/-! `lem:contactdrift`'s grade key: the highest root's coroot pair
...
the fundamental keys' `θ`-coroot reads (`lem:contactdrift`'s grade key;
...
instance instCorner2 (t : gentable.Table) : Decidable (cveeRead t) :=
  inferInstanceAs (Decidable (_ = _))

/-- The grade key at a coroot list: the dual fold's read
`G = Σ_i m_i c^∨_i`, one natural per label (`lem:contactdrift`). -/
...
`θ`-content's coroot vector (`lem:contactdrift`'s move cap). -/
...
(`lem:contactdrift`: the reduced shape's top row at the one-member
site). -/
...
own division read back (`lem:contactdrift`; `con:sertables`). -/
theorem cvee_sum : ∀ t : gentable.Table, gentable.shapeRead t →
  ...
  | Or.inl hD0 =>
    rw [hD0] at hres hsum
    rw [hsum, hres, ground.divZero, ground.famFold_zero _
      (fun i => ground.divZero _) (List.range t.rank)]
  | Or.inr hDpos =>
...
the key folds the halved summands whole (`lem:contactdrift`). -/
...
theorem coeffSum_le : ∀ (t : gentable.Table) (m : List Nat),
  ...
  intro t m hc hlen
  refine ground.sumNat_le_dotNat m (cvee t) ?_ ?_
...
at the sorted word (`lem:contactdrift`'s sorted-coordinate key reads). -/
...
coroot presentation's telescope at the sorted word (`lem:contactdrift`'s
...
telescope at the sorted word (`lem:contactdrift`'s sorted-coordinate key
reads). -/
...
/-! `lem:contactdrift`'s move cap: a `θ` content is the unit content or a
...
root's fold at or beyond the unit (`lem:contactdrift`'s move cap at
...
products (`lem:contactdrift`; `con:gentable`). -/
...
upper wall (`lem:contactdrift`'s move cap). -/
...
balance partner at the swap (`lem:contactdrift`). -/
...
of the source's (`lem:contactdrift`). -/
theorem grade_move : ∀ (t : gentable.Table)
  ...
  have h3 : (BPair.ofNat (grade t c)).oneValue
    ...
    BPair.oneValue_trans
      (BPair.oneValue_symm (memberdata.natV_dot c (cvee t)))
      (BPair.oneValue_trans h1
        (BPair.oneValue_trans h2
          (BPair.add_congr (memberdata.natV_dot m (cvee t))
            (BPair.oneValue_refl _))))
...
/-! `lem:contactdrift`'s Casimir cap: the fundamental display
...
(`lem:contactdrift`'s Casimir cap at the member's fundamental reads). -/
...
instance instCorner5 (t : gentable.Table) (G : elim.Mat) (FN RN : Nat) :
...
`l(l+1)(K² + 1)` (`lem:contactdrift`; `lem:serstable`(i)). -/
...
the cleared member pair `[l(l+1) : casDenB l]` (`lem:contactdrift`;
...
below the key fold the displayed numerator below `l(l+1)(K² + 1)`,
the cleared member pair `[l(l+1) : casDenC l]` (`lem:contactdrift`;
`lem:serstable`(i)'s displayed value). -/
...
below the key fold the displayed numerator below `l(l+1)(K² + 1)`,
the cleared member pair `[l(l+1) : casDenD l]` (`lem:contactdrift`;
`lem:serstable`(i)'s displayed value). -/
...
below `d²(d+1)(K² + 1)` (`lem:contactdrift`; `def:c2hat`). -/
...
the adjoint's clearing the consumer's own read (`lem:contactdrift`'s
...
/-! `lem:contactdrift`'s per-key counts: the labels at one key value are
...
entry bound the halved summands' own (`lem:contactdrift`). -/
...
theorem keyAll_length : ∀ (t : gentable.Table) (g : Nat),
  ...
  refine Nat.le_trans (ground.length_filter_le _ _) ?_
...
theorem dimM_cap : ∀ (t : gentable.Table) (m : List Nat),
  ...
  refine Nat.le_trans (ground.divLeSelf _ _) ?_
  refine Nat.le_trans
...
member reads `A`, `p` (`lem:contactdrift`). -/
...
count's unit (`lem:contactdrift`'s dimension-weighted family, cleared at
the scale triple). -/
...
(`lem:contactdrift`'s magnetic display, cleared). -/
...
below the gap's multiple of the whole power (`lem:contactdrift`'s
...
Pascal descent closing the weighted geometric sum (`lem:contactdrift`'s
...
/-! `lem:contactdrift`'s Rayleigh close: the dimension-weighted family
...
/-- `lem:contactdrift`'s Rayleigh read: at the family's fold data every
...
comparison, one cross-multiplication (`lem:contactdrift`'s
`g(σ)^{q+1} ≤ C_g^{q+1} σ`). -/
...
/-! `lem:contactdrift`'s rate composition assembles one scale's data: the located drift's margin
...
two sides, and the atoms' positive ties close `lem:contactdrift`'s
...
comparison, the shared grading weight cancelling (`lem:contactdrift`'s
