...
theorem pin1 : elecDiag (dataA 2) square 12 = [12] := by decide +kernel
...
theorem pin2 : (elecDiag (dataA 2) square 32).length = 2 := by
  rw [hDiag32]; rfl

/-! The diagonal's positivity: every occupied content positive, the
unit line the kernel beside the list. -/

theorem pin3 : diagPos (dataA 2) square 12 := by decide +kernel
theorem pin4 : diagPos (dataA 2) square 32 := by
...
theorem pin5 : diagPos (tabulate (dataA 2) 12) thetaG 12 := by
