...
theorem pin1 : multRead hD (idMat 2) 15 16 17 16 1 8 1
    (inertia.oneSplit [⟨2, 1⟩, ⟨34, 1⟩]) (inertia.oneSplit [⟨1, 2⟩, ⟨32, 1⟩]) := by decide +kernel

/-! A wide bracket is refused at the separation's width clause:
the whole segment `[1:2, 7:2]` holds both roots, its width beyond
the polynomial's `δ`. -/

theorem pin2 : ¬ multRead hD (idMat 2) 1 2 7 2 3 1 2
    (inertia.oneSplit [⟨2, 1⟩, ⟨6, 1⟩]) (inertia.oneSplit [⟨1, 6⟩, ⟨1, 2⟩]) := by decide +kernel
...
theorem pin3 : symRead (⟨hTh, rfl⟩ : SqMat 4) (⟨gSwap, rfl⟩ : SqMat 4)
    ⟨psi4, rfl⟩ := by decide +kernel

theorem pin4 : (dotN psi4 (matVec (matMul (transposeM gSwap)
