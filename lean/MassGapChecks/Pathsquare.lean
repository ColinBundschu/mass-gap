...
theorem pin1 : removals [1, 1, 0] = [[0, 1, 0], [2, 0, 0]] := rfl
theorem pin2 : removals [2, 0] = [[1, 0]] := rfl
theorem pin3 : removals [0, 1] = [[1, 0]] := rfl

/-! Both removal arms at one shape: `[1, 0, 1]` covers the
shortened head `[0, 0, 1]` and the grown column `[1, 1, 0]`. -/

theorem pin4 : removals [1, 0, 1] = [[0, 0, 1], [1, 1, 0]] := rfl

/-! The path counts, the standard tableau counts. -/

theorem pin5 : pathCount [2, 0] = 1 := by decide +kernel
theorem pin6 : pathCount [1, 1] = 2 := by decide +kernel
theorem pin7 : pathCount [1, 1, 0] = 2 := by decide +kernel
theorem pin8 : pathCount [0, 2, 0] = 2 := by decide +kernel
theorem pin9 : pathCount [2, 1] = 3 := by decide +kernel
theorem pin10 : pathCount [0, 0, 1] = 1 := by decide +kernel
theorem pin11 : pathCount [1, 1, 1] = 16 := by decide +kernel
theorem pin12 : pathCount [1, 0, 1] = 3 := by decide +kernel

/-! The recursion's two reads at instances: the seed at the vacant
degree and the step as the removals' fold. -/

theorem pin13 : pathCount [0, 0] = 1 := pathCount_vac [0, 0] rfl
theorem pin14 : pathCount [1, 0, 1]
...
theorem pin15 : ground.famFold Nat.add 0 (fun s => pathCount s * pathCount s)
    (places.allShapes 3 3) = 6 := by decide +kernel
theorem pin16 : ground.famFold Nat.add 0 (fun s => pathCount s * pathCount s)
    (places.allShapes 3 3) = ground.factorial 3 := squareSum 3 3 (Nat.le_refl 3)
theorem pin17 : ground.famFold Nat.add 0 (fun s => pathCount s * pathCount s)
    (places.allShapes 4 3) = ground.factorial 3 := squareSum 4 3 (by decide +kernel)
theorem pin18 : ground.famFold Nat.add 0 (fun s => pathCount s * pathCount s)
    (places.allShapes 2 3) = 5 := by decide +kernel
theorem pin19 : ¬ ground.famFold Nat.add 0 (fun s => pathCount s * pathCount s)
    (places.allShapes 2 3) = ground.factorial 3 := by decide +kernel
theorem pin20 : ground.famFold Nat.add 0 (fun s => pathCount s * pathCount s)
...
theorem pin21 : ground.countOf [1, 1, 0] (pieri.row [0, 1, 0])
    = ground.countOf [0, 1, 0] (removals [1, 1, 0]) :=
  row_removals [0, 1, 0] [1, 1, 0]
theorem pin22 : ground.countOf [2, 0] (pieri.row [1, 0])
    = ground.countOf [1, 0] (removals [2, 0]) :=
  row_removals [1, 0] [2, 0]
theorem pin23 : ground.countOf [1, 0] (pieri.row [1, 1, 0])
    = ground.countOf [1, 1, 0] (removals [1, 0]) :=
  row_removals [1, 1, 0] [1, 0]
theorem pin24 : ground.countOf [1, 0] (pieri.row [1, 1, 0]) = 0 := by decide +kernel
theorem pin25 : places.degree [0, 1, 0] + 1 = places.degree [1, 1, 0]
