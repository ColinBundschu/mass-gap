...
instances.  The boundary defect at the label calculus of fundamental
...
second jump from level `10` to level `12` at counts `1` and `2`,
each site datum's split committed at the cleared descent's
congruence — the level-`0` datum at the exchanged key order with its
mixed 2×2 block — and the level `11` inside the brackets' gap at
both cross-added comparisons, with the refusals at level `9` below
the gap, at level `14` beyond it, and at exchanged jump brackets —
a wide ground bracket reaching past the second jump's lower one —
the ordering conjunct alone refusing.  The jump read's order and
gap conjuncts share their refusal: the counting function is
monotone in its level, so reversed levels force the vacant gap, the
joint record committed at the two conjuncts' one forgery.
...
congruence — the columns `e₀`, `(-2, 5, 0)`, `(4, -10, 21)` at the
diagonal `5, 105, 1785` — with the isolating refusal at the scalar
`-1`, every binder but the comparison standing while the count
reads one, the forged split at a last pivot moved by one, and the
certificate parting the conclusion at a lower-side leading block,
the count reading one against the upper-side conclusion.  The count telescope at the four-slab
chain whose head diagonal sits at the sum's unit over the constant
`(5, 2)` tail: the tail pivots `-84/85, 85/21, 21/5, 5` at the
witnesses `42/85, 10/21, 2/5`, the whole datum's split at the mixed
2×2 block `[[0,2],[2,5]]` ahead of `5` and `105`, and the count the
head slab's own fold; the isolating refusal at the two-slab chain of
tail `-1`, whose dropped tail refuses the comparison with the counts
parting at `1`, the forged tail split beside it.  The located region
at the growing diagonal `(6, 10, 14)` against the constant stencil
coupling — the level-`3` datum inside the region, the level-`5`
datum beyond its edge at the leading row's least content.  The
constant-coefficient instance at the deck relation's cleared root
pair `(4, 1)` over the chord `5` and the clearing `2`, the pivot
fixed point `4` with its transfer `1/2` closing both steps of the
constant recursion.
Clause (iii)'s batteries ride the deck pivot's slab tier.  The bulk
chain of head diagonal `3` over the constant `(5, 2)` tail carries
its tail pivots `171/85, 85/21, 21/5, 5` at the witnesses
`42/85, 10/21, 2/5`, the bulk data constant beyond the first slab
with a forged third slab parting that fold alone, and the deck
pivot `Y = 4` the ball certificate's center at the witness
`Cc = 1/2` — the pivot identities the fixed point's step, the
contraction `λ = 1/2`, the floor at `y0 = 3`, `ρ = 1`, the vacant
defect at `ρ0 = 1/2` with `λ∘ = 2/3` — and the three dropped
pivots inside the ball at the deviations `1/21, 1/5, 1`.  The band
clause reads the deck symbol `2w - 5` through its cleared-variable
monic `y - 5` at the doubled magnitude's endpoints `(-4, 4)`, the
root outside at both counts the sum's unit; the slab step
assembles whole at the chain with its isolating refusals — the
forged tie, the starved contraction, the forged witness, the
forged floor at the re-solved tie, the forged pairing split (the
band's semantic forgery starving the ball's floor beside it, the
joint tie the mathematics' own), the forged pivot list at its
...
while the dropped pivot `-1` refuses every radius-one ball about
the deck pivot, the ball hypothesis the parting binder.
The emitted record's tier re-reads the divisor list at the head pencil
`[[10 - 2x, 1], [1, 10 - 2x]]` over the divisor `4x² - 40x + 99`:
five cells at the counts `0, 1, 2, 2, 2` and four boundaries — the
two jumps at the divisor's own roots and the comparison boundaries
at the stated displays `2x - 13` and `3x - 22` — with the refusals
at the truncated boundary list, at a forged leading count, at a
forged leading split, at a comparison boundary retagged as a jump,
...
whose count reads one at every triple, with `θ` the label one
against the unit label `0` — the delta reads the sum's unit while
the count reads one. -/

private def FV : fusion.Data Nat :=
...
matrix `(0, 11, 31)` on the diagonal with the coupling `-1` at the
outer keys, and the comparison datum is the identity at order three,
so the site datum at the level pair `[x : y]` is `H - (x - y) G`.

The four site data with their splits.  At level `-1` the datum
`H + G` splits at the clearing of the outer coupling to
`diag(1, 12, 31)`, the upper side throughout.  At level `0` the
datum is `H` itself, whose leading entry sits at the sum's unit: the
key order exchanged to `(0, 2, 1)` at a congruence of determinant
`-1` puts the mixed 2×2 block `[[0,-1],[-1,31]]` ahead of `11`, one
lower-side unit at the block's own determinant.  At level `10` the
datum `H - 10 G` splits at the clearing of the outer coupling to
`diag(-10, 1, 2110)`, and at level `12` the datum `H - 12 G` to
`diag(-12, -1, 2748)` — the counts `0, 1, 1, 2`. -/

private def wH : Mat :=
  pairpencil.pencilH 1 1 [0, 12, 32] (elim.idMat 3)
...
private def spA : Split 3 :=
  ⟨⟨[[⟨2, 1⟩, u, ⟨2, 1⟩], [u, ⟨2, 1⟩, u], [u, u, ⟨2, 1⟩]], rfl⟩,
   ⟨[[⟨2, 1⟩, u, ⟨1, 2⟩], [u, ⟨2, 1⟩, u], [u, u, ⟨2, 1⟩]], rfl⟩,
   [.one ⟨2, 1⟩, .one ⟨13, 1⟩, .one ⟨32, 1⟩], 0, rfl⟩

private def spB : Split 3 :=
  ⟨⟨[[⟨2, 1⟩, u, u], [u, u, ⟨2, 1⟩], [u, ⟨2, 1⟩, u]], rfl⟩,
   ⟨[[⟨1, 2⟩, u, u], [u, u, ⟨1, 2⟩], [u, ⟨1, 2⟩, u]], rfl⟩,
   [.two u ⟨1, 2⟩ ⟨32, 1⟩, .one ⟨12, 1⟩], 0, rfl⟩

private def spC : Split 3 :=
  ⟨⟨[[⟨2, 1⟩, u, ⟨2, 1⟩], [u, ⟨2, 1⟩, u], [u, u, ⟨1, 11⟩]], rfl⟩,
   ⟨[[⟨1, 11⟩, u, ⟨1, 2⟩], [u, ⟨1, 11⟩, u], [u, u, ⟨2, 1⟩]], rfl⟩,
   [.one ⟨1, 11⟩, .one ⟨2, 1⟩, .one ⟨2111, 1⟩], 0, rfl⟩

private def spD : Split 3 :=
  ⟨⟨[[⟨2, 1⟩, u, ⟨2, 1⟩], [u, ⟨2, 1⟩, u], [u, u, ⟨1, 13⟩]], rfl⟩,
...
reaching level `24`, past the second jump's lower bracket at level
`12`, both jump reads standing at their own splits with the level
comparisons beside them. -/

private def spE : Split 3 :=
  ⟨⟨[[⟨2, 1⟩, u, u], [u, u, ⟨2, 1⟩], [u, ⟨2, 1⟩, u]], rfl⟩,
   ⟨[[⟨1, 2⟩, u, u], [u, u, ⟨1, 2⟩], [u, ⟨1, 2⟩, u]], rfl⟩,
   [.two ⟨1, 25⟩ ⟨1, 2⟩ ⟨8, 1⟩, .one ⟨1, 14⟩], 0, rfl⟩

private def spF : Split 3 :=
  ⟨⟨[[⟨2, 1⟩, u, ⟨1, 2⟩], [u, ⟨2, 1⟩, u], [u, u, ⟨33, 1⟩]], rfl⟩,
...
forgery: the counting function is monotone in its level, so an
...
`2, 4, 2` down the rows — sits at or below its own diagonal entry.
The refusals part at one row apiece: a diagonal `1` beneath the fold
...
paired to the sum's unit against its predecessors, at `det T = 105`
with the adjugate the solved witness; the congruence's diagonal
reads `5, 105, 1785`, the upper side throughout.  The theorem
consumes the comparison and the split, the kernel reading the count
beside it. -/

private def spP : Split 3 :=
  ⟨⟨[[⟨2, 1⟩, ⟨1, 3⟩, ⟨5, 1⟩], [u, ⟨6, 1⟩, ⟨1, 11⟩], [u, u, ⟨22, 1⟩]], rfl⟩,
...
while the comparison refuses, and the count reads one against the
upper-side conclusion. -/

private def SN : Mat := [[⟨1, 2⟩]]
...
comparison stands, and its count reads one against the upper-side
conclusion. -/
...
transpose read alone refuses, and at the coordinates `(1, -2)` the
priced form parts — the cross term enters once against the fold's
two magnitudes. -/
...
/-! The count telescope at the four-slab chain: the head slab's
diagonal at the sum's unit over the constant `(5, 2)` tail, the
walk's shape at the dominance tail.  The tail recursion's pivots are
`-84/85, 85/21, 21/5, 5` at the witnesses `42/85, 10/21, 2/5`, each
slab's certificate the scalar congruence, and the dropped tail
sub-chain assembles the tridiagonal above — its comparison the
theorem's binder, its split that datum's own.  The whole chain's
split takes the columns `e₀`, `e₁`, `(-1, 0, 1, 0)`, `(2, 0, -2, 5)`
at `det T = 5`: the mixed 2×2 block `[[0,2],[2,5]]` ahead of `5` and
`105`, one lower-side unit at the block's determinant `-4`.  The
count is the head slab's own fold, one on both sides. -/

private def diagT : List Mat :=
  [[[u]], [[⟨6, 1⟩]], [[⟨6, 1⟩]], [[⟨6, 1⟩]]]
private def offT : List Mat := [[[⟨3, 1⟩]], [[⟨3, 1⟩]], [[⟨3, 1⟩]]]

private def xsT : List greenprod.MatQ :=
  [⟨[[⟨1, 85⟩]], 85⟩, ⟨[[⟨86, 1⟩]], 21⟩, ⟨[[⟨22, 1⟩]], 5⟩, ⟨[[⟨6, 1⟩]], 1⟩]
private def rsT : List greenprod.MatQ :=
  [⟨[[⟨43, 1⟩]], 85⟩, ⟨[[⟨11, 1⟩]], 21⟩, ⟨[[⟨3, 1⟩]], 5⟩]

private def spsT : List ((k : Nat) × Split k) :=
  [⟨1, ⟨⟨[[⟨2, 1⟩]], rfl⟩, ⟨[[⟨2, 1⟩]], rfl⟩, [.one ⟨1, 85⟩], 0, rfl⟩⟩,
   ⟨1, ⟨⟨[[⟨2, 1⟩]], rfl⟩, ⟨[[⟨2, 1⟩]], rfl⟩, [.one ⟨86, 1⟩], 0, rfl⟩⟩,
   ⟨1, ⟨⟨[[⟨2, 1⟩]], rfl⟩, ⟨[[⟨2, 1⟩]], rfl⟩, [.one ⟨22, 1⟩], 0, rfl⟩⟩,
   ⟨1, ⟨⟨[[⟨2, 1⟩]], rfl⟩, ⟨[[⟨2, 1⟩]], rfl⟩, [.one ⟨6, 1⟩], 0, rfl⟩⟩]

private def SP4 : Split 4 :=
  ⟨⟨[[⟨2, 1⟩, u, ⟨1, 2⟩, ⟨3, 1⟩], [u, ⟨2, 1⟩, u, u],
     [u, u, ⟨2, 1⟩, ⟨1, 3⟩], [u, u, u, ⟨6, 1⟩]], rfl⟩,
   ⟨[[⟨6, 1⟩, u, ⟨6, 1⟩, u], [u, ⟨6, 1⟩, u, u],
...
carries the tail recursion at the pivots `4, -1` with the witness
`-2`, every certificate reading its slab and the whole datum
splitting at the mixed block `[[0,2],[2,-1]]` of determinant `-4`;
the dropped tail refuses the comparison at the lower-side scalar and
the counts part, `1` against the head slab's fold at the sum's unit.
The forged tail split moves that scalar to the upper side and refuses
the congruence's read. -/

private def diagN : List Mat := [[[u]], [[⟨1, 2⟩]]]
private def offN : List Mat := [[[⟨3, 1⟩]]]
private def xsN : List greenprod.MatQ := [⟨[[⟨5, 1⟩]], 1⟩, ⟨[[⟨1, 2⟩]], 1⟩]
private def rsN : List greenprod.MatQ := [⟨[[⟨1, 3⟩]], 1⟩]

private def spsN : List ((k : Nat) × Split k) :=
  [⟨1, ⟨⟨[[⟨2, 1⟩]], rfl⟩, ⟨[[⟨2, 1⟩]], rfl⟩, [.one ⟨5, 1⟩], 0, rfl⟩⟩,
   ⟨1, ⟨⟨[[⟨2, 1⟩]], rfl⟩, ⟨[[⟨2, 1⟩]], rfl⟩, [.one ⟨1, 2⟩], 0, rfl⟩⟩]

private def SPN : Split 2 :=
  ⟨⟨idMat 2, rfl⟩, ⟨idMat 2, rfl⟩, [.two u ⟨3, 1⟩ ⟨1, 2⟩], 0, rfl⟩
private def spTN : Split 1 :=
  ⟨⟨[[⟨2, 1⟩]], rfl⟩, ⟨[[⟨2, 1⟩]], rfl⟩, [.one ⟨1, 2⟩], 0, rfl⟩
...
`(6, 10, 14)` the growing content against the constant stencil
coupling `2`, the site datum shifted down by the level at the
identity gram.  At level `3` the diagonal reads `3, 7, 11` and every
row's fold `2, 4, 2` sits beneath it, the datum inside the region.
At level `5` the diagonal reads `1, 5, 9`: the leading row carries
the least content and its fold `2` parts the comparison alone, the
region's edge one linear read per level. -/

private def SG3 : Mat :=
  [[⟨4, 1⟩, ⟨3, 1⟩, u], [⟨3, 1⟩, ⟨8, 1⟩, ⟨3, 1⟩], [u, ⟨3, 1⟩, ⟨12, 1⟩]]
private def SG5 : Mat :=
  [[⟨2, 1⟩, ⟨3, 1⟩, u], [⟨3, 1⟩, ⟨6, 1⟩, ⟨3, 1⟩], [u, ⟨3, 1⟩, ⟨10, 1⟩]]
private def HG : Mat :=
...
relation's cleared root pair `(4, 1)` at the chord `5` over the
clearing `2` — the product read `4 = 2²`, the chord read `4 + 1 = 5`,
and the matched squares `17 = 5² - 2·2²` against the family's
cleared evaluation.  The fixed point `4` with its transfer `1/2`
closes both steps of the constant recursion, `X* R* = 2` the
coupling's transpose and `X* + 2 R* = 5` the diagonal, and the
transfer taken at the clearing reads the root pair's interior member
...
over the constant `(5, 2)` tail: the tail recursion's pivots
`171/85, 85/21, 21/5, 5` at the witnesses `42/85, 10/21, 2/5`, the
bulk data constant beyond the first slab, and the deck pivot `Y = 4`
the interior solvent's partner at `X = 1/2` — the pivot identities
`Y X = 2` and `Y + 2 X = 5`, the solvent identity `2X² + 2 = 5X`,
the deck polynomial's symmetry, the graph read at `V₁ = 1`,
`V₂ = 1/2`, the floor at `y0 = 3`, `ρ = 1` with its site datum at
the sum's unit, and the three dropped pivots `85/21, 21/5, 5` inside
the ball of radius one around the center at the deviations
...
private def diagB : List Mat :=
  [[[⟨4, 1⟩]], [[⟨6, 1⟩]], [[⟨6, 1⟩]], [[⟨6, 1⟩]]]
private def offB : List Mat := [[[⟨3, 1⟩]], [[⟨3, 1⟩]], [[⟨3, 1⟩]]]
private def xsB : List greenprod.MatQ :=
  [⟨[[⟨172, 1⟩]], 85⟩, ⟨[[⟨86, 1⟩]], 21⟩, ⟨[[⟨22, 1⟩]], 5⟩,
   ⟨[[⟨6, 1⟩]], 1⟩]
private def rsB : List greenprod.MatQ :=
  [⟨[[⟨43, 1⟩]], 85⟩, ⟨[[⟨11, 1⟩]], 21⟩, ⟨[[⟨3, 1⟩]], 5⟩]
private def AB : Mat := [[⟨6, 1⟩]]
private def BB : Mat := [[⟨3, 1⟩]]
...
theorem pin110 : ¬ (((List.range diagB.length).all (fun i =>
    decide (i < 1) || decide (matOneValue
      (ground.getAt []
        [[[⟨4, 1⟩]], [[⟨6, 1⟩]], [[⟨7, 1⟩]], [[⟨6, 1⟩]]] i)
        AB))) = true) := by decide +kernel

/-! The band clause at that datum reads the symbol through its
cleared-variable monic: the deck symbol at `m = 1` is `2w - 5`, its
cleared display the monic `y - 5` at `y = 2w`, and the segment
count at the doubled magnitude's endpoints `(-4, 4)` reads the root
outside — the pairing `[[1]]` against the crossing pencil `[[9]]`,
both counts the sum's unit.  The slab step assembles whole at the
chain, with the isolating refusals: the forged tie `λ∘ = 1/3`, the
starved contraction `λ = 1/3`, the forged witness `Cc = 1/3` (the
pivot identities parting), the floor at `y0 = 5` with the tie
re-solved at `λ∘ = 3/5`, the band certificate at a forged pairing
split — the band clause's semantic forgery, a bulk datum whose
symbol root sits inside the segment, starves the ball's floor
beside it at the marginal contraction, so the certificate forgery
is the isolating refusal, the joint tie the mathematics' own; the
deck polynomial's symmetry is every deck datum's, its read the
display's verification with its refusals at the forged polynomials
— the forged pivot list at the recomputed cap, the forged third
slab at that chain's own recomputed tail with the ball widened to
`ρ = 6/5` at `y0 = 2`, and the forged third coupling at its own
chain, every further conjunct standing each time. -/
...
private def offW : List Mat := [[[⟨2, 1⟩]], [[⟨2, 1⟩]]]
private def xsW : List greenprod.MatQ :=
  [⟨[[⟨22, 1⟩]], 8⟩, ⟨[[⟨9, 1⟩]], 3⟩, ⟨[[⟨4, 1⟩]], 1⟩]
private def rsW : List greenprod.MatQ :=
  [⟨[[⟨4, 1⟩]], 8⟩, ⟨[[⟨2, 1⟩]], 3⟩]
private def xsW' : List greenprod.MatQ :=
  [⟨[[⟨14, 1⟩]], 8⟩, ⟨[[⟨9, 1⟩]], 3⟩, ⟨[[⟨4, 1⟩]], 1⟩]
private def ysW : List greenprod.MatQ :=
  [⟨[[⟨4, 1⟩]], 1⟩, ⟨[[⟨9, 1⟩]], 3⟩, ⟨[[⟨22, 1⟩]], 8⟩]
...
private def rsV : List greenprod.MatQ :=
  [⟨[[⟨5, 1⟩]], 11⟩, ⟨[[⟨2, 1⟩]], 4⟩]
private def ysV : List greenprod.MatQ :=
  [⟨[[⟨4, 1⟩]], 1⟩, ⟨[[⟨9, 1⟩]], 3⟩, ⟨[[⟨30, 1⟩]], 8⟩]
...
the ball of radius one around the deck pivot `4` at the floor
...
the divisor is the determinant `4x² - 40x + 99`, non-monic and
squarefree at its own Bézout `2 D + (5 - x) D' = -2`, and its
cleared-variable monic at `y = 4x` is `y² - 40y + 396`, the roots
`18` and `22` the divisor's `9/2` and `11/2` rescaled by the top's
magnitude. -/

private def spTwo (a b c : BPair) : Split 2 :=
  ⟨⟨idMat 2, rfl⟩, ⟨idMat 2, rfl⟩, [.two a b c], 0, rfl⟩

private def sR : split.PMat :=
  [[[⟨11, 1⟩, ⟨1, 3⟩], [⟨2, 1⟩]], [[⟨2, 1⟩], [⟨11, 1⟩, ⟨1, 3⟩]]]

private def ctR : cellcount.DivCert :=
  ⟨[], [⟨100, 1⟩, ⟨1, 41⟩, ⟨5, 1⟩], [⟨1, 41⟩, ⟨9, 1⟩], [⟨3, 1⟩],
...
monic is `[[2, 40], [40, 808]]`, its congruence the column pair
`e₀`, `(-20, 1)` at the diagonal `2, 8`, the upper side throughout.
The five samples sit at `x = 4, 5, 6, 7, 15/2` — the balance-pair
numerators over the shared weight `2` — with the evaluated head
matrices `[[4,2],[2,4]]`, `[[0,2],[2,0]]`, `[[-4,2],[2,-4]]`,
`[[-8,2],[2,-8]]`, `[[-10,2],[2,-10]]`, each split at the identity
congruence's one mixed block, the counts `0, 1, 2, 2, 2`.  The four
boundaries carry their bracket pencils at the composite endpoints:
the two jumps straddle the cleared roots `18` on `[16, 20]` and
`22` on `[20, 24]`, each pencil's determinant `-12288` the one
reversal; the dominance boundary at `2x - 13` and the deck boundary
at `3x - 22` read the divisor's count at the sum's unit across
`[24, 28]` and `[28, 30]` while their own cleared monics `y - 13`
at `y = 2x` and `y - 22` at `y = 3x` straddle their roots, the
scalar pencils `-4` and `-2` the one reversal apiece. -/

private def spHDR : Split 2 :=
  ⟨⟨[[⟨2, 1⟩, ⟨1, 21⟩], [u, ⟨2, 1⟩]], rfl⟩,
   ⟨[[⟨2, 1⟩, ⟨21, 1⟩], [u, ⟨2, 1⟩]], rfl⟩,
   [.one ⟨3, 1⟩, .one ⟨9, 1⟩], 0, rfl⟩

private def cellsR : List (BPair × Nat × Split 2) :=
  [(⟨9, 1⟩, 0, spTwo ⟨5, 1⟩ ⟨3, 1⟩ ⟨5, 1⟩),
   (⟨11, 1⟩, 1, spTwo u ⟨3, 1⟩ u),
   (⟨13, 1⟩, 2, spTwo ⟨1, 5⟩ ⟨3, 1⟩ ⟨1, 5⟩),
   (⟨15, 1⟩, 2, spTwo ⟨1, 9⟩ ⟨3, 1⟩ ⟨1, 9⟩),
   (⟨16, 1⟩, 2, spTwo ⟨1, 11⟩ ⟨3, 1⟩ ⟨1, 11⟩)]

private def spB1 : Split 2 := spTwo ⟨33, 1⟩ ⟨769, 1⟩ ⟨18049, 1⟩
private def spB2 : Split 2 := spTwo ⟨33, 1⟩ ⟨513, 1⟩ ⟨7809, 1⟩
private def spB3 : Split 2 := spTwo ⟨289, 1⟩ ⟨5377, 1⟩ ⟨100993, 1⟩
private def spB4 : Split 2 := spTwo ⟨673, 1⟩ ⟨12865, 1⟩ ⟨248449, 1⟩

private def pDomR : poly.Poly := [⟨1, 14⟩, ⟨3, 1⟩]
private def pDeckR : poly.Poly := [⟨1, 23⟩, ⟨4, 1⟩]
...
/-! The count forge isolates the pointwise read: the leading cell's
count moved to one at its own split, whose reversal fold reads the
sum's unit, every boundary beside it at its binding. -/
...
cell's split moved off the evaluated head at one entry, the stored
count kept. -/
...
the record: the part's Bézout combination moved off the displayed
constant, the part itself kept, so every cell and every boundary
reads its own data beside the one refused conjunct. -/

private def ctF : cellcount.DivCert :=
  ⟨[], [⟨100, 1⟩, ⟨1, 41⟩, ⟨5, 1⟩], [⟨1, 41⟩, ⟨9, 1⟩], [⟨3, 1⟩],
...
demands one root of the divisor's own cleared monic between its
flanking samples, and the stored bracket pencil reads the sum's
unit there — the segment `[24, 28]` off the roots `18` and `22`. -/
...
the display `3x - 22` in place of `2x - 13`.  The divisor's own
straddle stands at the sum's unit, the comparison's Hermite pairing
and bracket pencil are that display's own at the flanking samples —
the cleared segment `[18, 21]` with the pencil `[[16]]` — and the
segment count reads the sum's unit there, the root `22` outside, so
the binding's stated one is the conjunct that parts. -/
