...
theorem pin5 : weylchar.topRead [2, 0]
    ((blockSpan [2, 0]).map HVec.content) := by decide +kernel
theorem pin6 : weylchar.supportRead [2, 0]
    ((blockSpan [2, 0]).map HVec.content) := by decide +kernel
theorem pin7 : weylchar.symRead 2
    ((blockSpan [2, 0]).map HVec.content) := by decide +kernel
theorem pin8 : weylchar.recRead [2, 0]
    ((blockSpan [2, 0]).map HVec.content) := by decide +kernel
...
theorem pin10 : occupancyAt (blockSpan [1, 1, 0]) (rowList [1, 1, 0]) = 1 := by
  rw [blockcount.span110_pin]; decide +kernel
theorem pin11 : occupancyAt (blockSpan [1, 1, 0]) [1, 1, 1] = 2 := by
  rw [blockcount.span110_pin]; decide +kernel
theorem pin12 : occupancyAt (blockSpan [2, 0]) [1, 1] = 1 := by decide +kernel
theorem pin13 : occupancyAt (blockSpan [1, 1, 0]) [3, 0, 0] = 0 := by
  rw [blockcount.span110_pin]; decide +kernel
theorem pin14 : weylchar.domBy [1, 1, 1] (rowList [1, 1, 0]) := by decide +kernel
theorem pin15 : weylchar.domBy [1, 1] (rowList [2, 0]) := by decide +kernel

/-! The height fold's reads: the top's value with the drop at an
occupied lowering. -/

theorem pin16 : ht (rowList [1, 1, 0]) = 5 := by decide +kernel
theorem pin17 : ht (units.moveDn 0 (rowList [1, 1, 0])) + 1
...
theorem pin18 : lowerspan.ht [1, 1] < lowerspan.ht (blockcount.moveAt 0 1 [1, 1])
...
theorem pin19 : lowerspan.ht [1, 1] < lowerspan.ht (blockcount.moveAt 0 1 [1, 1]) :=
...
theorem pin20 : (∀ v ∈ blockSpan [1, 1, 0], blockcount.sized v)
    ∧ blockcount.indepAll (blockSpan [1, 1, 0])
    ∧ blockcount.closedAt (blockSpan [1, 1, 0]) 3 :=
  spanReads [1, 1, 0]
theorem pin21 : (∀ v ∈ blockSpan [2, 0], blockcount.sized v)
