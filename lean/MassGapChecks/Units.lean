...
open ground places elim

/-! The Leibniz action. -/

theorem pin1 : unitAct 0 1 [0, 1] = [[0, 0]] := rfl
theorem pin2 : unitAct 0 1 [1, 1] = [[0, 1], [1, 0]] := rfl
theorem pin3 : unitAct 1 0 [0, 0] = [[1, 0], [0, 1]] := rfl

/-! The commutator table on `V^⊗2` at two letters:
`[E_01, E_10] + E_11 = E_00`, and `[E_00, E_01] = E_01`. -/

theorem pin4 : matOneValue
    (matAdd
...
theorem pin8 : matOneValue (matUnitAt [2, 1] [1, 2] 0 1)
    (matUnit (monomialsAt [2, 1]) (monomialsAt [1, 2]) 0 1) := by decide +kernel
theorem pin9 : matOneValue (matUnitAt [1, 2, 0] [1, 1, 1] 1 2)
    (matUnit (monomialsAt [1, 2, 0]) (monomialsAt [1, 1, 1]) 1 2) := by
  decide +kernel

/-! The stacked raisings' key counts at the adjoint content. -/

theorem pin10 : (stackedRaise [1, 1]).length = 1 := rfl
theorem pin11 : (stackedRaise [2, 0]).length = 0 := rfl
...
theorem pin15 : (ground.getAt BPair.unit
  ...
  decide +kernel
theorem pin16 : ground.countOf [0, 2, 1] (unitAct 0 1 [1, 2, 1])
    = ground.countOf [1, 2, 1] (unitAct 1 0 [0, 2, 1]) :=
  countOf_unitAct_swap 0 1 [1, 2, 1] [0, 2, 1]
theorem pin17 : ground.countOf [0, 2, 1] (unitAct 0 1 [1, 2, 1]) = 1 := by
  decide +kernel
theorem pin18 : ground.countOf [1, 2, 1] (unitAct 2 2 [1, 2, 1])
    = if [1, 2, 1] = [1, 2, 1] then ground.countOf 2 [1, 2, 1]
      else 0 :=
  countOf_unitAct_diag 2 [1, 2, 1] [1, 2, 1]
theorem pin19 : ground.countOf [1, 2, 1] (unitAct 2 2 [1, 2, 1]) = 1 := by
  decide +kernel
...
theorem pin20 : ground.countOf [0, 1]
      ((unitAct 1 0 [0, 1]).flatMap (unitAct 0 1))
...
theorem pin21 : ground.countOf [0, 1]
...
theorem pin25 : matOneValue (transposeM (matUnitAt [2, 0] [1, 1] 0 1))
    (matUnitAt [1, 1] [2, 0] 1 0) := by decide +kernel

theorem pin26 : matOneValue (transposeM (matUnitAt [3, 0] [2, 1] 0 1))
...
theorem pin27 : matNull (matUnitAt [1, 1] [1, 1] 0 1) := by decide +kernel

theorem pin28 : ¬ matNull (matUnitAt [2, 0] [1, 1] 0 1) := by decide +kernel
...
theorem pin29 : ¬ (places.rowList (moveDn 0 [1])
    = ground.bumpAt 1 (places.rowList [1])) := by decide +kernel
theorem pin30 : ¬ (places.rowList (moveDn 0 [0, 1])
    = ground.bumpAt 1 (places.rowList [0, 1])) := by decide +kernel
theorem pin31 : places.rowList (moveDn 0 [1, 1])
    = ground.bumpAt 1 (places.rowList [1, 1]) := by decide +kernel
theorem pin32 : places.rowList (moveDn 0 [1, 1])
